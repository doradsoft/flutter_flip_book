...
class FlipBookState extends State<FlipBook> with TickerProviderStateMixin, AutomaticKeepAliveClientMixin<FlipBook> {
  ...
  double _delta = 0;
  bool _dragUpdating = false;
  final Map<int, bool> _leafBuilding = {};
  ...
  Widget leafBuilder(BuildContext context, Leaf leaf) {
    _leafBuilding[leaf.index] = true;
    try {
      if ((!leaf.isCover && (leaf.index - controller.currentLeaf.index).abs() > widget.bufferSize) ||
          (leaf.isCover && widget.coverAspectRatio.value == widget.leafAspectRatio.value)) {
      ...
      _leafBuilding[leaf.index] = false;
    }
  }
  ...
  void _onDragUpdate(DragUpdateDetails details) {
    if (controller.animating || _dragUpdating || _leafBuilding.containsValue(true)) {
      return;
    }
    _dragUpdating = true;
    try {
